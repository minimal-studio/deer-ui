...
export default class Carousel3D extends Component<Carousel3DProps, {
  ...
  activeIdx: number;
  bannerTotalWidth: number;
  rotateIdx: number;
}> {
  defaultRotateX = getIsMobile() ? '200px' : '1600px'
  ...
  startLoop() {
    ...
    this.timer = setInterval(() => {
       ...
       * 如果不在当前的标签，就不渲染
       */
      if (!document.hidden) this.roll(activeIdx - 1);
    }, this.freq);
  }
  ...
  render() {
    const { carouselRotateY, activeIdx } = this.state;
