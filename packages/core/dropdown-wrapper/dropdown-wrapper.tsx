...
export class DropdownWrapper extends React.PureComponent<
...
> {
  ...
  setOutSideContainer = (isMobile) => {
    if (this._outside && !dropdownContainerDOM) {
      dropdownContainerDOM = setDOMById(
        dropdownContainerID,
        `__dropdown-menu outside`
      );
    }
  };
  ...
  getOverlayDOM = () => {
    ...
    const dropdownCom = (
      <TransitionGroup component={null}>
        <CSSTransition key={transitionKey} classNames="drop-menu" timeout={200}>
          {isShow ? (
            <div
  ...
  overlayRender = () => {
    return this._outside
      ? dropdownContainerDOM &&
          ReactDOM.createPortal(this.getOverlayDOM(), dropdownContainerDOM)
      : this.getOverlayDOM();
  };
  ...
  getDefaultChild = (menuTitle) => {
    ...
    return (
      <div className="display-menu">
        <div className="display-title">{menuTitle}</div>
        {this._withInput && (
          <input
            type="text"
            ref={this.saveInput}
