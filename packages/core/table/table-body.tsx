...
/** 兼容旧的 TableKeyMapper */
export type TableKeyMapper = TableColumn[];

export type TableDataRows = DataRows;
...
export interface TableProps extends ColumnFilterProps {
  /** 需要重命名为 columns */
  keyMapper?: TableKeyMapper;
  /** 定义 table 的 columns */
  columns: TableColumns;
  /** records -> dataRows */
  records?: TableDataRows;
  /** 数据源 */
...
export class Table extends ColumnFilter<TableProps, State> {
  ...
  fixedLeftGroup: TableColumn[] = [];

  fixedRightGroup: TableColumn[] = [];
  ...
  toggleAllItems(allCheck) {
    ...
    if (!allCheck) {
      ...
      this.getDataRows().forEach((item, idx) => { nextCheckedItems[idx] = item; });
    }
  ...
  render() {
    ...
    const isAllCheck = hasRecord && (checkedItemLen === dataRows.length);
